...
public class AccountAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		case CODE_INSERT:
			...
			dto_ACC = new DTO_ACCOUNT(ID, PW, HOSPI_NAME, ORG_NUM, COMP_NUM, PHONE, MAIL, ADDR, ADDR_DETAIL, REG_DATE,
					MOD_DATE);
		...
		case CODE_UPDATE:
			...
			dto_ACC = new DTO_ACCOUNT(Integer.parseInt(AC_ID_U), ID_U, PW_U, HOSPI_NAME_U, ORG_NUM_U, COMP_NUM_U,
					PHONE_U, MAIL_U, ADDR_U, ADDR_DETAIL_U, REG_DATE_U, MOD_DATE_U);
		...
		case CODE_LOGIN:
			...
			String PW_L = request.getParameter("PW");
			
			dto_ACC = new DTO_ACCOUNT(ID_L, PW_L);
			DTO_ACCOUNT resultVO = dao.LOGIN_ACCOUNT(dto_ACC, conn);
			HttpSession sess_IN = request.getSession();

			sess_IN.setAttribute("userID", ID_L);
			
			// request.setAttribute("result", new Boolean(result));
			request.setAttribute("ID", resultVO);
			break;
		case CODE_LOGOUT:
			HttpSession sess_OUT = request.getSession();
			sess_OUT.removeAttribute("userID");
			break;
		case CODE_TEST:
			break;
		}
