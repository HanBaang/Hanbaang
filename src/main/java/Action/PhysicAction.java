...
public class PhysicAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		case CODE_INSERT:
			...
			dto_PH = new DTO_PHYSIC(Integer.parseInt(CH_ID), Integer.parseInt(TR_ID), NAME, PH_TYPE, PART_CODE,
					Boolean.valueOf(INSUR));
		...
		case CODE_UPDATE:
			...
			dto_PH = new DTO_PHYSIC(Integer.parseInt(PH_ID_U), Integer.parseInt(CH_ID_U), Integer.parseInt(TR_ID_U),
					PH_TYPE_U, NAME_U, PART_CODE_U, Boolean.valueOf(INSUR_U));
			result = dao.INSERT_PHYSIC(dto_PH, conn);
			request.setAttribute("result", new Boolean(result));
			break;
		}
